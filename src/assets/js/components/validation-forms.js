...
export function initFormsValidator() {
	...
	Parsley.addValidator("maxFileSize", {
		validateString: function (_value, maxSize, parsleyInstance) {
			if (!window.FormData) {
				alert(
					"Вы заставляете всех разработчиков в мире съеживаться. Обновите свой браузер!"
				);
				return true;
			}
			const files = parsleyInstance.$element[0].files,
				filesSize = [...files].reduce(
					(courent, file) => courent + file.size,
					0
				);
	...
	Parsley.addValidator("fileFormats", {
		validateString: function (_value, formats, parsleyInstance) {
			...
			const formatsNames = [...parsleyInstance.$element[0].files].map((file) =>
				file.name.split(".").pop()
			);

			return formatsNames
				.map((format) => formats.toLowerCase().includes(format))
				.every((access) => !!access);
		},
