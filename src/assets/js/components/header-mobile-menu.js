...
export default () => {
	...
	const div = document.createElement("div"),
		mobileMenuWrap =
			header.querySelector(".mobile_menu__wrap") || div.cloneNode(),
		mobileMenubtn =
			header.querySelector(".mobile_menu__btn") || div.cloneNode(),
		...
		closeMenu = () => {
			...
			header
				.querySelector("." + subMenuActiveClass)
				?.classList.remove(subMenuActiveClass);
		};
	...
	for (let i = 0; i < 3; i++) {
		mobileMenubtn.append(div.cloneNode());
	}
	...
	if (!header.querySelector(".mobile_menu__wrap"))
